...
def cruzar_dados_api(df_local, dados_api_externa):
    if dados_api_externa:
        ...
        if 'agenteDoSucesso' in df_api_externa.columns:
            colunas_desejadas.append('agenteDoSucesso')
        df_cruzado = pd.merge(
            df_local,
        ...
        if 'status' in df_cruzado.columns:
            df_cruzado['status'] = df_cruzado['status'].fillna('')
    ...
    if dados:
        ...
        try:
            ...
            if response.status_code in [200, 201]:
                print(f"Dados enviados com sucesso para {url}. Total de registros: {len(dados)}")
            else:
...
# Data de hoje para a variável `toUpdated`
